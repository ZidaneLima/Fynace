...
from utils.api_client import login_with_google, get_resumo, post_transacao


st.set_page_config(page_title="Fynace", layout="wide")

# --- Captura token e email da URL ---
query_params = st.query_params
user_email = query_params.get("email", [None])[0] if "email" in query_params else None
...
def logout():
    ...
    st.success("Você saiu com sucesso. Atualize a página para fazer login novamente.")

# --- Se não logado, mostra tela de login ---
if "token" not in st.session_state or not st.session_state["token"]:
    st.title("Fynace - Seu Controle Financeiro Pessoal")
...
categoria = st.sidebar.selectbox(
    ...
    if tipo == "Despesa" else ["Salário", "Freelancer", "Outros"]
)
...
components.html(open("components/cards_component.html").read().replace(
    "{{ item.title }}", "Saldo Total"
).replace(
    "{{ item.value }}", f"R$ {resumo['saldo_restante']:.2f}"
), height=120)

# Gráfico
df = pd.DataFrame(resumo["detalhes"])
fig = px.bar(df, x="Categoria", y="Valor", color="Tipo", title="Distribuição Financeira")
